...
void* producer(void* args) {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (buffer_index == BUFFER_SIZE) {
            pthread_cond_signal(&buffer_full);  //Utiliza signal
        }
...
int main(void) {
    ...
    pthread_t consumer_thread;  // Variável única (não array)
    ...
    pthread_create(&consumer_thread, NULL, consumer, NULL);  // DIFERENÇA: Apenas 1 thread
